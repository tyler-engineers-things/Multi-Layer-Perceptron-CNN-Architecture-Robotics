...
board_qpos_id = model.joint(name="bowl_joint")
board_qpos_addr = board_qpos_id.qposadr[0] # start index in data.qpos
...
position_new = [0.3 + values[0], 0.0 + values[1], 0.04175]
desired_pos = [0.5, 0.0, 0.04175]

# Update qpos: [x, y, z, qw, qx, qy, qz]
data.qpos[board_qpos_addr:board_qpos_addr+3] = position_new
data.qpos[board_qpos_addr+3:board_qpos_addr+7] = [quat_new[3], quat_new[0], quat_new[1], quat_new[2]]
mujoco.mj_forward(model, data)
...
checkpoint_path = os.path.join(os.path.dirname(__file__), "push_sim_checkpoint.pt") # change file based on real or sim
...
for t in range(maximum_simulation_steps):
    ...
    current_bowl_posi = data.qpos[board_qpos_addr:board_qpos_addr+3]
    print("Current bowl pose:", current_bowl_posi)

    # the distance is smaller than 0.05, then considered as success.
    if np.linalg.norm(desired_pos - current_bowl_posi) <= 0.05:
        break
